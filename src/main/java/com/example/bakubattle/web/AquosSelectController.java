...
@Slf4j
@Controller
@RequestMapping("/aquos")
@SessionAttributes("player")
public class AquosSelectController {
    ...
    @Autowired
    public AquosSelectController(JdbcBakuganRepository jdbcBakuganRepository, JdbcPlayerRepository jdbcPlayerRepository) {
    ...
    public String loadBakuganSelect(Model model){
        ...
        try{
            ...
            e.printStackTrace();
            log.info("Error: not able to pull the aquos bakugans from a database.");
            return "error";
        }
    }
    ...
    private void displayResultGet(List<Bakugan> bakugans){
        log.info("----------------------------- Aquos Select GET Window Opened  ---------------------------------");
        ...
        log.info("----------------------------- Aquos Select GET Window Closed  ---------------------------------");
    }

    private void displayResultPost(Bakugan bakugan, Player player){
        log.info("----------------------------- Aquos Select POST Window Opened  ---------------------------------");
        log.info("Player's name: "+player.getName());
        log.info("Bakugan's name: "+bakugan.getName());
        log.info("Bakugan's further data: "+ bakugan);
        log.info("----------------------------- Aquos Select POST Window Closed  ---------------------------------");
    }
}
