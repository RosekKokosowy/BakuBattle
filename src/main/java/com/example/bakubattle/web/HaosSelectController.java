...
public class HaosSelectController {
    ...
    @Autowired
    public HaosSelectController(JdbcBakuganRepository jdbcBakuganRepository, JdbcPlayerRepository jdbcPlayerRepository) {
    ...
    public String loadBakuganSelect(Model model){
        ...
        try{
            ...
            e.printStackTrace();
            log.info("Error: not able to pull the haos bakugans from a database.");
            return "error";
        }
    }
    ...
    private void displayResultGet(List<Bakugan> bakugans){
        log.info("----------------------------- Haos Select GET Window Opened  ---------------------------------");
        ...
        log.info("----------------------------- Haos Select GET Window Closed  ---------------------------------");
    }

    private void displayResultPost(Bakugan bakugan, Player player){
        log.info("----------------------------- Haos Select POST Window Opened  ---------------------------------");
        log.info("Player's name: "+player.getName());
        log.info("Bakugan's name: "+bakugan.getName());
        log.info("Bakugan's further data: "+ bakugan);
        log.info("----------------------------- Haos Select POST Window Closed  ---------------------------------");
    }
}
