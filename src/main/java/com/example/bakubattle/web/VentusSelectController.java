...
@Slf4j
@Controller
@RequestMapping("/ventus")
@SessionAttributes("player")
public class VentusSelectController {
    ...
    @Autowired
    public VentusSelectController(JdbcBakuganRepository jdbcBakuganRepository, JdbcPlayerRepository jdbcPlayerRepository) {
    ...
    public String loadBakuganSelect(Model model){
        ...
        try{
            ...
            return "ventus_select";
        } catch (Exception e){
            e.printStackTrace();
            log.info("Error: not able to pull the ventus bakugans from a database.");
            return "error";
        }
    }
    ...
    private void displayResultGet(List<Bakugan> bakugans){
        log.info("----------------------------- Ventus Select GET Window Opened  ---------------------------------");
        ...
        log.info("----------------------------- Ventus Select GET Window Closed  ---------------------------------");
    }

    private void displayResultPost(Bakugan bakugan, Player player){
        log.info("----------------------------- Ventus Select POST Window Opened  ---------------------------------");
        log.info("Player's name: "+player.getName());
        log.info("Bakugan's name: "+bakugan.getName());
        log.info("Bakugan's further data: "+ bakugan);
        log.info("----------------------------- Ventus Select POST Window Closed  ---------------------------------");
    }
}
