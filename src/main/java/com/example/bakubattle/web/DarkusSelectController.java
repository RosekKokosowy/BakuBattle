...
@Slf4j
@Controller
@RequestMapping("/darkus")
@SessionAttributes("player")
public class DarkusSelectController {
    ...
    @Autowired
    public DarkusSelectController(JdbcBakuganRepository jdbcBakuganRepository, JdbcPlayerRepository jdbcPlayerRepository) {
    ...
    public String loadBakuganSelect(Model model){
        ...
        try{
            ...
            return "darkus_select";
        } catch (Exception e){
            e.printStackTrace();
            log.info("Error: not able to pull the darkus bakugans from a database.");
            return "error";
        }
    }
    ...
    private void displayResultGet(List<Bakugan> bakugans){
        log.info("----------------------------- Darkus Select GET Window Opened  ---------------------------------");
        ...
        log.info("----------------------------- Darkus Select GET Window Closed  ---------------------------------");
    }

    private void displayResultPost(Bakugan bakugan, Player player){
        log.info("----------------------------- Darkus Select POST Window Opened  ---------------------------------");
        log.info("Player's name: "+player.getName());
        log.info("Bakugan's name: "+bakugan.getName());
        log.info("Bakugan's further data: "+ bakugan);
        log.info("----------------------------- Darkus Select POST Window Closed  ---------------------------------");
    }
}
