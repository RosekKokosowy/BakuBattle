...
@Slf4j
@Controller
@RequestMapping("/subterra")
@SessionAttributes("player")
public class SubterraSelectController {
    ...
    @Autowired
    public SubterraSelectController(JdbcBakuganRepository jdbcBakuganRepository, JdbcPlayerRepository jdbcPlayerRepository) {
    ...
    public String loadBakuganSelect(Model model){
        ...
        try{
            ...
            return "subterra_select";
        } catch (Exception e){
            e.printStackTrace();
            log.info("Error: not able to pull the subterra bakugans from a database.");
            return "error";
        }
    }
    ...
    private void displayResultGet(List<Bakugan> bakugans){
        log.info("----------------------------- Subterra Select GET Window Opened  ---------------------------------");
        ...
        log.info("----------------------------- Subterra Select GET Window Closed  ---------------------------------");
    }

    private void displayResultPost(Bakugan bakugan, Player player){
        log.info("----------------------------- Subterra Select POST Window Opened  ---------------------------------");
        log.info("Player's name: "+player.getName());
        log.info("Bakugan's name: "+bakugan.getName());
        log.info("Bakugan's further data: "+ bakugan);
        log.info("----------------------------- Subterra Select POST Window Closed  ---------------------------------");
    }
}
